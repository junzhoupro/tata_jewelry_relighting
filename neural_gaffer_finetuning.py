...
from dataset.dataset_relighting_finetuning import NeuralGafferTrainingDataLoader, NeuralGafferTrainingData
...
warnings.filterwarnings("ignore",
                        message="The config attributes {'cc_projection': ['pipeline_zero1to3', 'CCProjection']} were passed to Neural_Gaffer_StableDiffusionPipeline, but are not expected and will be ignored. Please verify your model_index.json configuration file.")
...
def log_validation(validation_dataloader,
                   vae, image_encoder, feature_extractor,
                   unet, args, accelerator, weight_dtype,
    ...
    for valid_step, batch in tqdm(enumerate(validation_dataloader)):
        ...
        envir_map_target_hdr_npy = 0.5 * (
                    np.array(target_envmap_hdr.permute([0, 2, 3, 1]).cpu(), dtype=np.float32) + 1.0)
        envir_map_target_ldr_npy = 0.5 * (
                    np.array(target_envmap_ldr.permute([0, 2, 3, 1]).cpu(), dtype=np.float32) + 1.0)
        ...
        concatenated_image = np.concatenate(
            [input_image_npy_new, gt_image_npy_new, prediction_image_sample0_new, prediction_image_sample1_new,
             envir_map_target_ldr_npy_new, envir_map_target_hdr_npy_new], axis=1)
    ...
    logs = {"lpips_loss/{}".format(split): mean_lpips_loss, "ssim_loss/{}".format(split): mean_ssim_loss,
            "PSNR/{}".format(split): mean_psnr}
...
def CLIP_preprocess(x):
    ...
    x = kornia.geometry.resize(x.to(torch.float32), (224, 224), interpolation='bicubic', align_corners=True,
                               antialias=False).to(dtype=dtype)  # not bf16
    x = (x + 1.) / 2.
...
def main(args):
    ...
    image_encoder = CLIPVisionModelWithProjection.from_pretrained(args.pretrained_model_name_or_path,
                                                                  subfolder="image_encoder", **loading_kwargs)
    ...
    conv_in_16 = torch.nn.Conv2d(16, unet.conv_in.out_channels, kernel_size=unet.conv_in.kernel_size,
    ...
    def print_model_info(model):
        ...
        if accelerator.is_main_process:
            ...
            print("non-learnable parameters(M): ",
    ...
    train_dataset = NeuralGafferTrainingData(
        ...
        from_img_dir=args.from_img_dir_train, # added
    ...
    training_dataset_unseen_lighting = NeuralGafferTrainingData(
        ...
        img_dir=args.train_img_dir, # added
        lighting_dir=args.train_lighting_dir, # added
        from_img_dir=args.from_img_dir_train,  # added
        lighting_per_view=8,
        total_view=4,
        image_transforms=image_transforms,
    ...
    if accelerator.is_main_process:
        ...
        accelerator.init_trackers(args.tracker_project_name, config=tracker_config,
                                  init_kwargs={"wandb": {"name": f'{output_basename}'}})
    ...
    if args.resume_from_checkpoint:
        ...
        if path is None:
            ...
            # print("output_dir, path", args.output_dir, path)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                sobel_img = batch["sobel"].to(dtype=weight_dtype) # added
                # pose = batch["T"].to(dtype=weight_dtype)
                ...
                sobel_img = torch.cat((sobel_img, sobel_img), dim=0)
                ...
                sobel_latents = vae.encode(sobel_img).latent_dist.mode().detach()
                ...
                noisy_latents = noise_scheduler.add_noise(gt_latents.to(dtype=torch.float32),
                                                          noise.to(dtype=torch.float32), timesteps).to(
                ...
                if do_classifier_free_guidance:  # support classifier-free guidance, randomly drop out 5%
                    ...
                    img_prompt_embeds = _encode_image(image_encoder, input_image, gt_latents.device, gt_latents.dtype,
                                                      False)
                    ...
                    prompt_embeds = _encode_image_without_pose(image_encoder, input_image, gt_latents.device,
                                                               weight_dtype, False)
                ...
                latent_model_input = torch.cat(
                    [noisy_latents,
                     img_latents,
                     # sobel_latents, # added
                     target_envir_map_hdr_latents,
                     target_envir_map_ldr_latents], dim=1)

                # Predict the noise residual
                # print("latent_model_input ", latent_model_input.shape)
                # print("prompt_embeds ", prompt_embeds.shape)
                model_pred = unet(
                    latent_model_input,                  # torch.Size([4, 16, 32, 32])
                    timesteps,
                    encoder_hidden_states=prompt_embeds, # torch.Size([4, 1, 768])
                ).sample
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if validation_dataloader_random_light_condition is not None and (
                            global_step % args.validation_steps == 0 or global_step == (100 + initial_global_step)):
                        if args.use_ema:
                    ...
                    if validation_dataloader_unseen_lighting is not None and (
                            global_step % args.validation_steps == 0 or global_step == (100 + initial_global_step)):
                        if args.use_ema:
                            # Store the UNet parameters temporarily and load the EMA parameters to perform inference.
                            ema_unet.store(unet.parameters())
                    ...
                    if validation_dataloader_seen_lighting is not None and (
                            global_step % args.validation_steps == 0 or global_step == (100 + initial_global_step)):
                        if args.use_ema:
                            # Store the UNet parameters temporarily and load the EMA parameters to perform inference.
                            ema_unet.store(unet.parameters())
                        ...
                        if args.use_ema:
                            ...
                            ema_unet.restore(unet.parameters())
                        # step_log.update(temp_log)
                    if training_dataloader_unseen_lighting is not None and (
                            global_step % args.validation_steps == 0 or global_step == (100 + initial_global_step)):
                        if args.use_ema:
                            # Store the UNet parameters temporarily and load the EMA parameters to perform inference.
                            ema_unet.store(unet.parameters())
