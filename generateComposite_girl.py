...
def sam_out_nosave(predictor, input_image, *bbox_sliders):
    ...
    earring_mask = "/home/jz927/Documents/relighting/neuralgaffer/Neural_Gaffer/preprocessed_data2_final/mask/girlwithearrings.png"
    earring_mask = cv2.imread(earring_mask)
    earring_mask = (earring_mask > 128).astype(np.uint8)
    earring_mask = cv2.resize(earring_mask, dsize=(130, 130))
    print("mask: ",  np.max(earring_mask))

    pred_dir = "/home/jz927/Documents/relighting/neuralgaffer/Neural_Gaffer/real_data_relighting2/girlwithearrings/pred_image/girl_*.png"
    for name in glob(pred_dir):
        ...
        new_earring = cv2.resize(new_earring, dsize=(130, 130))
    ...
    # print(image.shape, new_earring.shape)
        resize_new_earring = cv2.resize(new_earring, dsize=(130, 130))
        background = image_copy[307:437, 279:409] * (1 - earring_mask)
        crop = resize_new_earring + background
        image_copy[307:437, 279:409] = crop
        image_copy = image_copy[0: 512, 0: 384]
    ...
    # earring_mask = np.pad(earring_mask, ((256, 0), (128, 0), (0, 0)))
        ...
        savedir = os.path.join("/home/jz927/Documents/relighting/neuralgaffer/Neural_Gaffer/video2/girl", savedir)
...
def run_dir(args):
    ...
    for name in image_names_temp:
        if name.endswith('.png') or name.endswith('.jpg') and name.startswith('girl'):
