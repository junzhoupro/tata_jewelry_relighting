...
def log_validation(validation_dataloader, vae, image_encoder, feature_extractor, unet, args, accelerator, weight_dtype, img_per_object=600, split="val"):
    ...
    for valid_step, batch in tqdm(enumerate(validation_dataloader)):
        ...
        sobel_image = batch["sobel"].to(dtype=weight_dtype)
        ...
        for _ in range(args.num_validation_images): # sampled times
            with torch.autocast("cuda"):
                ...
                pipeline_output_images = pipeline(
                    input_imgs=input_image, prompt_imgs=input_image,
                    sobel_imgs=sobel_image,
                    first_target_envir_map=target_envmap_hdr,
                    second_target_envir_map=target_envmap_ldr,
                    poses=None, height=h, width=w,
...
def concatenate_images_horizontal(image1, image2, save_path='concatenated_image.png'):
    """
    Concatenates two PIL images horizontally and saves the result.

    Parameters:
    - image1 (PIL.Image.Image): The first image to concatenate.
    - image2 (PIL.Image.Image): The second image to concatenate.
    - save_path (str): The file path to save the concatenated image.

    Returns:
    - new_image (PIL.Image.Image): The concatenated image.
    """
    # Get dimensions of both images
    width1, height1 = image1.size
    width2, height2 = image2.size

    # New width is the sum of both image widths, height is the maximum height
...
def main(args):
    ...
    print(unet.conv_in.weight.shape)
    ...
    if args.resume_from_checkpoint:
        ...
        if path is None:
            ...
            # accelerator.load_state(os.path.join(args.output_dir, path, "unet")) # new trained
            accelerator.load_state(os.path.join(args.output_dir, path)) # neural gaffer trained
