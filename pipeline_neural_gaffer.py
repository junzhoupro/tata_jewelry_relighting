...
from transformers import CLIPFeatureExtractor, CLIPTextModel, CLIPTokenizer, CLIPVisionModelWithProjection, CLIPImageProcessor # added
import einops
...
class Neural_Gaffer_StableDiffusionPipeline(DiffusionPipeline):
    ...
    def _encode_image(self, image, device, num_images_per_prompt, do_classifier_free_guidance):
        ...
        # image = self.clip_image_processor(images=image, return_tensors="pt").pixel_values
    ...
    def prepare_img_latents(self, image, batch_size, dtype, device, generator=None, do_classifier_free_guidance=False):
        ...
        if isinstance(generator, list):
            init_latents = [
                self.vae.encode(image[i : i + 1]).latent_dist.sample(generator[i]) for i in range(batch_size) # sample
            ]
    ...
    def __call__(
        ...
        sobel_imgs: Union[torch.FloatTensor, PIL.Image.Image] = None,
        poses: Union[List[float], List[List[float]]] = None,
        first_target_envir_map: Union[torch.FloatTensor, PIL.Image.Image] = None,
        second_target_envir_map: Union[torch.FloatTensor, PIL.Image.Image] = None,
        ...
        sobel_latents = self.prepare_img_latents(sobel_imgs,
                                                 batch_size * num_images_per_prompt,
                                                 prompt_embeds.dtype,
                                                 device,
                                                 generator,
        ...
        first_target_envir_map_latents = self.prepare_img_latents(  
        ...
        second_target_envir_map_latents = self.prepare_img_latents(second_target_envir_map,
                                                            batch_size * num_images_per_prompt,
                                                            prompt_embeds.dtype,
                                                            device,
                                                            generator,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = torch.cat([
                    ...
                    img_latents,
                    # sobel_latents,
                    first_target_envir_map_latents,
                    second_target_envir_map_latents], dim=1)

                # print("latent_model_input ", latent_model_input.shape)
                # print("prompt_embeds ", prompt_embeds.shape)
                # predict the noise residual
                noise_pred = self.unet(latent_model_input, # torch.Size([8, 16, 32, 32])
                                       t,
                                       encoder_hidden_states=prompt_embeds #  torch.Size([8, 1, 768])
                                       ).sample
