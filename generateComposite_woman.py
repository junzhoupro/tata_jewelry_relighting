...
def sam_out_nosave(predictor, input_image, *bbox_sliders):
    ...
    earring_mask = "/home/jz927/Documents/relighting/neuralgaffer/Neural_Gaffer/preprocessed_data2_final/mask/womanwithearrings.png"
    earring_mask = cv2.imread(earring_mask)
    earring_mask = (earring_mask > 128).astype(np.uint8)
    earring_mask = cv2.resize(earring_mask, dsize=(108, 108))
    print("mask: ",  np.max(earring_mask))


    pred_dir = "/home/jz927/Documents/relighting/neuralgaffer/Neural_Gaffer/real_data_relighting2/womanwithearrings/pred_image/woman*.png"
    # print("------------------", glob(pred_dir))
    for name in glob(pred_dir):
        # print("name")
        image_copy = image
        # image_copy = np.pad(image_copy, ((0, 0), (0, 130), (0, 0)))
        # new_earring = name
        ...
        new_earring = cv2.resize(new_earring, dsize=(108, 108))
    ...
    # print(image.shape, new_earring.shape)
        resize_new_earring = cv2.resize(new_earring, dsize=(108, 108))
        background = image_copy[222:330, 248:356] * (1 - earring_mask)
        crop = resize_new_earring + background
        image_copy[222:330, 248:356] = crop
        # image_copy = image_copy[0: 720, 0: 540]

    # earring_mask = np.pad(earring_mask, ((256, 0), (128, 0), (0, 0)))
        print(image_copy.shape, new_earring.shape, earring_mask.shape)
        image_copy = cv2.cvtColor(image_copy, cv2.COLOR_RGB2BGR)
        savedir = os.path.basename(name)
        savedir = os.path.join("/home/jz927/Documents/relighting/neuralgaffer/Neural_Gaffer/video2/woman", savedir)
...
def run_dir(args):
    ...
    for name in image_names_temp:
        if name.endswith('.png') or name.endswith('.jpg') and name.startswith('woman'):
