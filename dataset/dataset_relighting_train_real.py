...
import torch
import torchvision
from examples.pytorch_hpc_example import train
...
class Relighting_DataLoader():
    def __init__(self, 
        ...
        # self.target_dir = target_dir # added
    ...
    def train_dataloader(self):
        dataset = Relighting_Data_Jewelry(lighting_dir=self.lighting_dir_train,
                                        img_dir=self.img_dir_train,
                                        target_dir=self.target_dir, # added
                                        total_view=self.total_view,
                                        lighting_per_view=10,
    ...
    def val_dataloader(self):
        dataset = Relighting_Data_Jewelry(lighting_dir=self.lighting_dir_val,
                                img_dir=self.img_dir_val,
                                target_dir=self.target_dir,  # added
                                total_view=self.total_view, lighting_per_view=10,
                                image_transforms=self.image_transforms)
        # sampler = DistributedSampler(dataset)
...
class Relighting_Data_Jewelry(Dataset):
    def __init__(self,
                 lighting_dir, 
                 img_dir,
                 target_dir, #added
        ...
        #added for target
        targets = os.listdir(os.path.join(self.target_dir))
        ...
        # print("1 ", self.lighting_dir_list)
        # print("2 ", self.lighting_per_view)
    ...
    def __getitem__(self, index):
        ...
        # print("object_inside_idx ", object_inside_idx)
        data = {}
        total_lighting_rotation_view = self.total_view # 12
        ...
        # print("? ", target_view_idx, target_lighting_idx)
        # print(len(self.lighting_dir_list), self.lighting_dir_list[2], self.lighting_dir_list[target_lighting_idx])

        data["envir_map_target_ldr"] = self.process_im(
            self.load_im(os.path.join(target_envir_map_lighting_dir,
                                      'LDR',
                                      f'{target_view_idx:02}.png')))
        data["envir_map_target_hdr"] = self.process_im(
            self.load_im(os.path.join(target_envir_map_lighting_dir,
                                      'HDR_normalized',
                                      f'{target_view_idx:02}.png')))
        ...
        data["target_envir_map_name"] = lighting_name #light name
        data["cond_img_name"] = os.path.basename(cond_image_path).split('.')[0]

        data['target_view_idx'] = target_view_idx
        # target added
        target_path = os.path.join(self.target_dir, data["cond_img_name"], f'{data["target_envir_map_name"]}.png')
        # print("target_path ", target_path)
        data['image_target'] = self.process_im(self.load_im(target_path))#added
        return data 
...
if __name__ == "__main__":
    ...
    # test dataloader
    dataloader = Relighting_Data_Jewelry(
        lighting_dir = '/home/jz927/Documents/relighting/neuralgaffer/Neural_Gaffer/jewelry2/light',
        img_dir = '/home/jz927/Documents/relighting/neuralgaffer/Neural_Gaffer/jewelry2/preprocessed_data/img',
        target_dir = '/home/jz927/Documents/relighting/neuralgaffer/Neural_Gaffer/jewelry2/real_data_relighting', # added
        lighting_per_view=16,
        total_view=1,
    ...
    for i, data in enumerate(train_loader):
        print("i: ==================", i)
        print(data['envir_map_target_ldr'].shape)
        numpy_array = data['envir_map_target_ldr'].permute(1, 2, 0).numpy()
        numpy_array = (numpy_array * 255).astype(np.uint8)
        image = Image.fromarray(numpy_array)
        image.save('envir_map_target_ldr.png')

        print(data['envir_map_target_hdr'].shape)
        numpy_array = data['envir_map_target_hdr'].permute(1, 2, 0).numpy()
        numpy_array = (numpy_array * 255).astype(np.uint8)
        image = Image.fromarray(numpy_array)
        image.save('envir_map_target_hdr.png')

        print(data['image_cond'].shape)
        numpy_array = data['image_cond'].permute(1, 2, 0).numpy()
        numpy_array = (numpy_array * 255).astype(np.uint8)
        image = Image.fromarray(numpy_array)
        image.save('image_cond.png')

        print("target_envir_map_name ", data['target_envir_map_name']) #light name
        print("cond_img_name ", data['cond_img_name']) #
        print("target_view_idx ", data['target_view_idx'])
        print("data['image_target'] ", data['image_target'].shape)
        numpy_array = data['image_target'].permute(1, 2, 0).numpy()
        numpy_array = (numpy_array * 255).astype(np.uint8)
        image = Image.fromarray(numpy_array)
        image.save('image_target.png')
