...
# from preprocessed_data6.applymask import img_dir


class NeuralGafferTrainingDataLoader():
    def __init__(self,
                 lighting_dir_train, img_dir_train,
                 from_img_dir_train, # added
        ...
        self.from_img_dir_train = from_img_dir_train
    ...
    def train_dataloader(self):
        dataset = NeuralGafferTrainingData(lighting_dir=self.lighting_dir_train,
                                           ...
                                           from_img_dir=self.from_img_dir_train, # added
                                           total_view=self.total_view,
                                           lighting_per_view=self.lighting_per_view,
                                           validation=False,
...
class NeuralGafferTrainingData(Dataset):
    def __init__(self,
        ...
        self.from_img_dir = from_img_dir # added
        print("img_dir ", self.img_dir)
        print("from_img_dir", self.from_img_dir)
    ...
    def get_T(self, target_RT, cond_RT):
        ...
        cond_orientation = torch.tensor(
            [theta_cond.item(), math.sin(azimuth_cond.item()), math.cos(azimuth_cond.item())])
        target_orientation = torch.tensor(
            [theta_target.item(), math.sin(azimuth_target.item()), math.cos(azimuth_target.item())])
    ...
    def load_im(self, path, color=None):
        ...
        if img.shape[-1] == 4:
            ...
            if color is not None:
                color = np.array(color) / 255.0  # Normalize color to [0,1] range
                img[alpha == 0, :3] = color  # Apply color to RGB channels only
    ...
    def sobel_pil(self, img_pil, thresh=50):
        '''Calculating the high-frequency map for a PIL image.'''
        # Convert PIL image to NumPy array
        img = np.array(img_pil.convert("RGB"))  # Ensure 3-channel RGB
        gray = np.array(img_pil.convert("L"))  # Convert to grayscale for edge detection
        ...
        scharr = cv2.addWeighted(sobel_X, 0.5, sobel_Y, 0.5, 0)  # Combine edges

        # Thresholding
        scharr[scharr < thresh] = 0

        # Expand scharr to match RGB shape
        scharr = np.stack([scharr] * 3, axis=-1)  # Convert to (H, W, 3)

        # Element-wise multiplication with the original image
        scharr = ((scharr.astype(np.float32) / 255) * img.astype(np.float32)).astype(np.uint8)

        # Convert NumPy array back to PIL image
        return Image.fromarray(scharr)
    ...
    def __getitem__(self, index):
        ...
        if self.validation:
            ...
            if self.relighting_only:
                # print("relighting only ", index)
                # randonly sample a view
                sampled_view = random.sample(range(total_view), 1)[0]
                index_target, index_cond = sampled_view, sampled_view
            else:
                index_target, index_cond = random.choices(range(total_view),
                                                          k=2)  # it is possible to have the same index value
            (lighting_idx_cond,
             lighting_idx_target,
             lighting_idx_another_target) = random.sample(range(total_lighting),3)  # without replacement, return unique values
            # print("lighting_idx_cond ", lighting_idx_cond)
            # print("lighting_idx_target ", lighting_idx_target)
            # print("lighting_idx_another_target ", lighting_idx_another_target)
            # print(" index_cond, ", index_cond)
            # 10% chance to make lighting_idx_cond (lighting condition if the input condition image) to be -1
            # if random.random() < 0.1:
            #     lighting_idx_cond = -1
        # print("dataset_type ", self.dataset_type, lighting_idx_cond)
        if self.dataset_type == 'unseen_object_with_random_area_light_condition':
            lighting_idx_cond = -1

        object_id = self.paths[index]
        index_target = object_id[-2:] # added
        filename = os.path.join(self.img_dir, object_id)
        # print("self.paths ", self.paths)
        # print("object_id ", object_id, ",", filename)
        ...
        if lighting_idx_cond == -1:
            ...
            # print("else ", self.from_img_dir, filename)
            # cond_image_path = glob(os.path.join(filename, '%03d_%03d_*.png' % (index_cond, lighting_idx_cond)))[0]
            cond_image_path = os.path.join(self.from_img_dir, object_id+".png")
            # print("cond_image_path ", cond_image_path)

        cond_im = self.process_im(self.load_im(cond_image_path))
        sobel_im = self.process_im(self.sobel_pil(self.load_im(cond_image_path)))

        # load target image
        # target_image_path = glob(os.path.join(filename, '%03d_%03d_*.png' % (index_target, lighting_idx_target)))[0]
        image_files = sorted(glob(os.path.join(filename, "*.*"))) # added,
        # print("image files ", len(image_files), " ", filename)
        # print("lighting_idx_target ", lighting_idx_target)
        target_image_path = os.path.join(filename, image_files[lighting_idx_target] ) # added,
        # print("target_image_path ", target_image_path)
        # target_hdr_name = Path(image_files[lighting_idx_target]).stem # added
        ...
        target_envir_map_name = os.path.basename(target_image_path)
        target_img_file_name = Path(target_image_path).stem
        # print("target_envir_map_name ", target_envir_map_name)
        # print("target_img_file_name ", target_img_file_name)
        # target_envir_map_ldr_path = os.path.join(self.preprocessed_lighting_dir, 'LDR', object_id, target_img_file_name)
        # target_envir_map_hdr_normalized_path = os.path.join(self.preprocessed_lighting_dir, 'HDR_rescaled', object_id,
        #                                                     target_img_file_name)
        target_envir_map_ldr_path = os.path.join(
            self.preprocessed_lighting_dir, target_img_file_name,
            'ldr', index_target+".png")
        target_envir_map_hdr_normalized_path = os.path.join(
            self.preprocessed_lighting_dir, target_img_file_name,
            'hdr', index_target+".png")
        # print("target_envir_map_ldr_path ", target_envir_map_ldr_path)
        # print("target_envir_map_hdr_normalized_path ", target_envir_map_hdr_normalized_path)
        ...
        if not self.validation:
            # print("??")
            # another_target_image_path = \
            # glob(os.path.join(filename, '%03d_%03d_*.png' % (index_target, lighting_idx_another_target)))[0]
            # print("lighting_idx_another_target ", lighting_idx_another_target)
            another_target_image_path = os.path.join(filename, image_files[lighting_idx_another_target])  # added,
            ...
            # another_target_envir_map_ldr_path = os.path.join(self.preprocessed_lighting_dir, 'LDR', object_id,
            #                                                  another_target_img_file_name)
            # another_target_envir_map_hdr_normalized_path = os.path.join(self.preprocessed_lighting_dir, 'HDR_rescaled',
            #                                                             object_id, another_target_img_file_name)
            # print("another_target_image_path ", another_target_image_path)
            another_target_envir_map_name = os.path.basename(another_target_image_path)
            another_target_img_file_name = Path(another_target_image_path).stem
            another_target_envir_map_ldr_path = os.path.join(
                self.preprocessed_lighting_dir, another_target_img_file_name,
                'ldr', index_target + ".png")
            another_target_envir_map_hdr_normalized_path = os.path.join(
                self.preprocessed_lighting_dir, another_target_img_file_name,
                'hdr', index_target + ".png")
            # print("another_target_envir_map_ldr_path ", another_target_envir_map_ldr_path)
            # print("another_target_envir_map_hdr_normalized_path ", target_envir_map_hdr_normalized_path)
...
if __name__ == "__main__":
    ...
    dataloader = NeuralGafferTrainingDataLoader(
        ...
        lighting_dir_train=f'/home/jz927/Documents/relighting/neuralgaffer/dataset3/light',
        # img_dir_train='/scratch/datasets/hj453/objaverse-rendering/filtered_V2/rendered_images_resized',
        img_dir_train='/home/jz927/Documents/relighting/neuralgaffer/dataset3/real_data_relighting',
        from_img_dir_train='/home/jz927/Documents/relighting/neuralgaffer/dataset3/preprocessed_data/img',
        # lighting_dir_val='/scratch/datasets/hj453/objaverse-rendering/filtered_V2/preprocessed_environment_resized_new/',
        lighting_dir_val=f'/home/jz927/Documents/relighting/neuralgaffer/training_lighting_data_subset',
        # img_dir_val='/scratch/datasets/hj453/objaverse-rendering/filtered_V2/rendered_images_resized',
        img_dir_val='/home/jz927/Documents/relighting/neuralgaffer/validation',
        batch_size=2,
